...
module.exports = function before (scope, sb) {
	...
	try {
		...
		if (files.length > 0) {
		  return sb.error('Couldn\'t create a new Sails app in "'+scope.rootPath+'" because `sails new` can only be called on an existing directory if that directory is empty.');
		}
	}
	catch (e) { }
