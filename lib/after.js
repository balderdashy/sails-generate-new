...
module.exports = function afterGenerate(scope, cb) {
  ...
  if (_.isUndefined(scope.templatePath) || !_.isString(scope.templatePath)) {
    return cb(new Error('Consistency violation: `templatePath` is a required string.'));
  }
    ...
    if (version.split('.')[0] <= 2 || scope.link === true) {
      ...
      async.auto({
        ...
        dependencies: ['create_node_modules', function (results, next) {
          ...
          var pjData = fsx.readJson(pathToPackageJson, function (err){
