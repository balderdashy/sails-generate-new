    ...
    if (version.split('.')[0] < 3 || scope.link === true) {
      ...
      async.auto({
        ...
        install_other_deps: ['create_sails_link', function(cb) {
          console.log(
            'You seem to be using NPM >= v3.0.0, which no longer supports symbolic links in the `node_modules/` folder.'+'\n'+
            'As of Feb. 2016, the Sails team is still working on a speedier solution to optimize `sails new`;'+'\n'+
            'but in the mean time, your new Sails app\'s dependencies will be set up using `npm install`.'+'\n'+
            'Please be patient with us, or downgrade to NPM < v3.0.0 in the mean time.  Thanks!'+'\n'+
            '\n'+
