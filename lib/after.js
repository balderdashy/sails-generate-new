...
module.exports = function afterGenerate(scope, cb) {
  ...
  exec('npm -v', function(err, stdout, stderr) {
    ...
    // --•
    // Otherwise, this is npm v3, so we'll have to actually `npm install` some
    // of the dependencies again locally.  Luckily, we can still at least symlink
    // the `sails` dependency.  But we can't safely symlink the others, at least
    // not yet.
    //
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    async.auto({
      ...
      create_node_modules: function create_node_modules(proceed) {
        fsx.mkdir(scope.rootPath + '/node_modules', proceed);
      },
